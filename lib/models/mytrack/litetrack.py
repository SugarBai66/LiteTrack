...
from lib.models.mytrack.relo_head import build_box_head as build_relo_head
...
class LiteTrack(nn.Module):
    ...
    def forward(self,template_feats, search ):
        # print("DEBUG: template_feats type:", type(template_feats))
        # print("DEBUG: search type:", type(search))
        x = self.backbone(z=template_feats,x=search, mode='x')
        # print("DEBUG: x type:", type(x))
        out = self.forward_head(x)
        # print("DEBUG: out type:", type(out))
        if self.onnx:
...
def build_LiteTrack(cfg, training=True):
    ...
    box_head = build_relo_head(cfg, hidden_dim)
