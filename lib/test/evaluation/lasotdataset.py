...
class LaSOTDataset(BaseDataset):
    ...
    def _get_sequence_list(self):
        # 根据 split 决定读取哪个文件,测试集，只读测试部分共280个序列
        filename = 'testing_set.txt'

        # 获取当前文件所在目录
        current_dir = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(self.env_settings.lasot_path, filename)

        if not os.path.isfile(file_path):
            raise FileNotFoundError(f"Sequence list file not found: {file_path}")
